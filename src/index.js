module.exports = function toReadable (number) {
    ...
    if(parseInt(x.slice(x.length-2,x.length-1))==0&&x>99) {
        ...
        res += parseTenNinety(x)
        res = res.slice(0,res.length-1)
        }
        else res+= parseTenNinety(x)+parseOneNine(x)
    }
    }
    console.log(res)
    
    function parseOneNine(num) {
        n = ['zero','one','two','three','four','five','six','seven','eight','nine']
        return n[parseInt(x.slice(x.length-1,x.length))]
    }

    function parseTenNinety(num) {
    ...
    function parseHundreds(num) {
        n = ['one hundred ','two hundred ','three hundred ','four hundred ','five hundred ', 'six hundred ','seven hundred ','eight hundred ','nine hundred ','nine hundred ']
